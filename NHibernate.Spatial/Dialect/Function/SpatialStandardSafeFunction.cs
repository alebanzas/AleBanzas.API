...
namespace NHibernate.Spatial.Dialect.Function
{
	...
	{
		protected int allowedArgumentsCount = 1;
		...
		public SpatialStandardSafeFunction(string name, int allowedArgsCount)
			: base(name)
		{
			this.allowedArgumentsCount = allowedArgsCount;
		}
		...
		{
            if (args.Count != allowedArgumentsCount)
            {
				throw new QueryException(string.Format("function '{0}' requires {1} arguments.", this.name, this.allowedArgumentsCount));
			}
		}
		...
		{
			get { return this.allowedArgumentsCount; }
		}
